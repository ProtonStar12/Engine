...
def integrate_simpson(a, b, equation, n_values):
    ...
    def calculate(points, parsed_eq):
        ...
        middle_odd_results = [
            parsed_eq.subs(x, point).evalf() for point in middle_points_odd
        ]
        middle_even_results = [
            parsed_eq.subs(x, point).evalf() for point in middle_points_even
        ]
        total_sum = (
            first_result
            + 4 * (sum(middle_odd_results))
            + 2 * (sum(middle_even_results))
            + last_result
        )
